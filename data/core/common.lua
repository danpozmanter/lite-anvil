...
local function fuzzy_match_items(items, needle, files)
  ...
  for _, item in ipairs(items) do
    local sort_text = tostring(item)
    local score = system.fuzzy_match(sort_text, needle, files)
    if score then
      table.insert(res, { text = item, sort_text = sort_text, score = score })
    end
  end
...
function common.path_suggest(text, root)
  ...
  local files, types = system.list_dir(path)
...
function common.dir_path_suggest(text, root)
  ...
  local files, types = system.list_dir(path == "" and root or path)
  if not files then return {} end
  local res = {}
  for i, name in ipairs(files) do
    local file = path .. name
    if types[i] == "dir" and file:lower():find(text:lower(), nil, true) == 1 then
      table.insert(res, file)
    end
  end
  return res
