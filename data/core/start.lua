-- this file is used by lite-anvil to setup the Lua environment when starting
...
if MACOS_RESOURCES then
  ...
  DATADIR = prefix and (prefix .. PATHSEP .. 'share' .. PATHSEP .. 'lite-anvil') or (EXEDIR .. PATHSEP .. 'data')
end
USERDIR = (system.get_file_info(EXEDIR .. PATHSEP .. 'user') and (EXEDIR .. PATHSEP .. 'user'))
       or os.getenv("LITE_USERDIR")
       or ((os.getenv("XDG_CONFIG_HOME") and os.getenv("XDG_CONFIG_HOME") .. PATHSEP .. "lite-anvil"))
       or (HOME and (HOME .. PATHSEP .. '.config' .. PATHSEP .. 'lite-anvil'))
