...
const CORE_SOURCE: &str = r##"require "core.strict"
require "core.regex"
...
local native_manifest = require "project_manifest"
local symbol_index = require "symbol_index"
local git_native = require "git_native"
local lsp_manager = require "lsp_manager"
local lsp_transport = require "lsp_transport"
local session_native = require "session_native"
...
local function load_session()
  local session = session_native.load()
  if session.legacy_path and system.get_file_info(session.legacy_path) then
    local ok, legacy = pcall(dofile, session.legacy_path)
    if ok and type(legacy) == "table" then
      session = legacy
      pcall(session_native.save, session)
    end
  end
  return session
end
...
local function save_session()
  ...
  session_native.save({
    ...
    recent_files = core.recent_files or {},
    treeview_size = treeview_size,
...
local function update_recents_project(action, dir_path_abs)
  ...
  core.recent_projects = session_native.update_recent_projects(core.recent_projects, action, dirname)
end

local function update_recent_file(path)
  local filename = common.normalize_volume(path)
  if not filename then return end
  core.recent_files = session_native.update_recent_files(core.recent_files or {}, filename)
end
...
local function release_project_resources(project)
  ...
  if native_project_model.invalidate then
    pcall(native_project_model.invalidate, project.path)
  end
  if native_manifest.invalidate then
...
local function clear_native_runtime_caches()
  ...
  if git_native.clear_cache then
    pcall(git_native.clear_cache)
  end
  if lsp_manager.clear_runtime_state then
    pcall(lsp_manager.clear_runtime_state)
  end
  if lsp_transport.clear_all then
...
local function is_bundled_plugin_path(path)
  if type(path) ~= "string" then
    return false
  end

  local bundled_prefix = DATADIR .. PATHSEP .. "plugins"
  return path:find(bundled_prefix, 1, true) == 1
end

local function is_generated_plugin_sidecar(path)
  if type(path) ~= "string" then
    return false
  end

  return path:match("%.luac$") ~= nil or path:match("%.lazy%.json$") ~= nil
end


function core.parse_plugin_details(path, file, mod_version_regex, priority_regex)
  if is_generated_plugin_sidecar(path) or is_generated_plugin_sidecar(file) then
    return nil
  end
  ...
  if is_bundled_plugin_path(path) or is_bundled_plugin_path(file) then
    version_match = true
  end
  return {
...
function core.register_session_load_hook(name, hook)
  assert(type(name) == "string", "session load hook name must be a string")
  assert(type(hook) == "function", "session load hook must be a function")
  core.session_load_hooks[name] = hook
  return hook
end


function core.register_session_save_hook(name, hook)
  assert(type(name) == "string", "session save hook name must be a string")
  assert(type(hook) == "function", "session save hook must be a function")
  core.session_save_hooks[name] = hook
  return hook
end


function core.plugin_open_doc(path_or_doc)
  if type(path_or_doc) == "string" then
    return core.root_view:open_doc(core.open_doc(path_or_doc))
  end
  return core.root_view:open_doc(path_or_doc)
end


function core.plugin_children()
  return core.root_view.root_node:get_children()
end


function core.plugin_get_node_for_view(view)
  return core.root_view.root_node:get_node_for_view(view)
end


function core.plugin_update_layout()
  return core.root_view.root_node:update_layout()
end


function core.plugin_root_size()
  return core.root_view.size.x, core.root_view.size.y
end


function core.plugin_enter_prompt(label, options)
  return core.command_view:enter(label, options)
end


function core.plugin_update_prompt_suggestions()
  return core.command_view:update_suggestions()
end


function core.plugin_add_status_item(item)
  return core.status_view:add_item(item)
end


function core.plugin_show_status_message(icon, color, text)
  return core.status_view:show_message(icon, color, text)
end


function core.plugin_show_status_tooltip(text)
  core.status_view.tooltip = { style.text, text }
  core.status_view.tooltip_mode = true
  return core.status_view.tooltip
end


function core.plugin_remove_status_tooltip()
  core.status_view.tooltip = {}
  core.status_view.tooltip_mode = false
end
